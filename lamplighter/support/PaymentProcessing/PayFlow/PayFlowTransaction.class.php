...
class PayFlowTransaction {
	...
	public function load_config() {
		...
		$this->data['USER'] = Config::Get('payflowpro.user');
		$this->data['PWD'] = Config::Get('payflowpro.password');
		$this->data['PARTNER'] = Config::Get('payflowpro.partner');
		
		if ( Config::Get('payflowpro.vendor') ) { 
			$this->data['VENDOR'] = Config::Get('payflowpro.vendor');
		}
		else {
		...
		if ( Config::Get('payflowpro.environment') ) {
			$this->environment = Config::Get('payflowpro.environment');
		}
	...
	public function send_transaction() {
		...
		try { 
			...
			Debug::Show( __METHOD__ . ' Sending: ' . $data_string );
	...
	public function response_handler( $response_arr ) {
		...
		try { 
			...
			Debug::Show( __METHOD__ . ' response=' . print_r( $response_arr, true) );
			Debug::Show( __METHOD__ . ' RESULT=' . $result_code );
