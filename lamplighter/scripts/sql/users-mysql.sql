...
CREATE TABLE `user_group_link` (
  ...
  CONSTRAINT `FK_user_groups_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON DELETE CASCADE ON UPDATE CASCADE,
...
DROP TABLE IF EXISTS `user_preference_types`;
...
DROP TABLE IF EXISTS `user_privilege_types`;
...
CREATE TABLE `users` (
  ...
  `user_password_encrypted` mediumtext,
